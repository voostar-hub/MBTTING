...
%>
	...
	<div class="write_form">
		<form name="qna_edit_frm" action="qna_edit_ok.jsp?qna_no=<%= qna_no %>&question_type=<%=q_type %>" method="post">
			<div class="writeForm_div">
				...
				<div class="write_selectbox">
					<select class="selectbox" name="question_type">
						<option value="fail">질문유형을 선택해주세요</option>
						<option value="board" 
								<c:if test="${type == board}">selected</c:if>>게시판 관련</option>
						<option value="chatting" 
								<c:if test="${type == chatting}">selected</c:if>>채팅 관련</option>
						<option value="etc" 
								<c:if test="${type == etc}">selected</c:if>>기타</option>
						
					</select>
				</div>
				<div class="title_div">
					<input type="text" name="qna_title" placeholder="내용을 입력해 주세요." value="<%=qq.getQna_title() %>" class="title" maxlength="30">
				</div>
				<div class="content_div">
					<textarea name="qna_content" class="content" placeholder="내용을 입력해 주세요."  maxlength="1000"><%= qq.getQna_content() %></textarea>
				</div>
				<div class="input_div">
					<div class="pwd_div">
						<input type="password" name="qna_pwd" class="pwd" placeholder="비밀번호를 입력해 주세요." maxlength="10">
					</div>
					<div class="input_btn_div">
						...
						<input type="button" value="수정 완료" onclick="qna_edit_check()"  class="write_ok_btn">
					</div>
				</div>
			</div>
		</form>
