...
BoardBean qq = manager.getQna(qna_no);
int qna_comment_cnt = manager.qnacountcomment(qna_no); //총 댓글 갯수
ArrayList<BoardBean> list = manager.qnaListCommnet(qna_no); // 댓글 화면에 뿌려주기
...
// textarea 엔터시 줄바꿈적용
String content = manager.getQna(qna_no).getQna_content();
content = content.replace("\n", "<br/>");

// 게시글 작성자는 본인 글 조회수 증가 막기
String writer = manager.getQna(qna_no).getUser_nick();
...
String qno = request.getParameter("qna_no");
//쿠키가 있을 경우 
if (cookies != null && cookies.length > 0) {
 for (int i = 0; i < cookies.length; i++){
     if (cookies[i].getName().equals("|" + qno + "|") ){ 
...
if (viewCookie == null) {    
 ...
 Cookie newCookie = new Cookie("|" + qno + "|", URLEncoder.encode(sessionNick, "UTF-8"));
 
 // 본인이 쓴글이 아닐경우 추가시킴 
 if(!writer.equals(logon_nick)) {
	    newCookie.setMaxAge(1*24*60*60);
	    response.addCookie(newCookie);
	  	manager.updateQnaCount(qna_no);
 }
} else {
 // 쿠키 값 받아옴.
...
%>
	...
	<header class="board-header">
		...
		%>
		<h1><a href="board_qna.jsp">Q & A - 게시판 관련</a></h1>
		<%
			} else if (q_type.equals("chatting")) {
		%>
		<h1><a href="chatting_qna.jsp">Q & A - 채팅 관련</a></h1>
		<%
			} else if (q_type.equals("etc")) {
		%>
		<h1><a href="etc_qna.jsp">Q & A - 기타</a></h1>
		<%
			} 
		%>
	</header>
	...
	<div class="board-title">
		...
		%>
				<div class="board_title_display">
					<div> 
						<h2><%=qq.getQna_title()%></h2>
					</div>	
					<div class="edit_del_div">
						<input type="button" value="수정" onclick="location.href='qna_edit.jsp?qna_no=<%= qna_no %>&question_type=<%= q_type%>'">
						&nbsp;
						<input type="button" value="삭제" onclick="location.href='qna_delete.jsp?qna_no=<%= qna_no%>&question_type=<%= q_type %>'">
					</div>
				</div>
		<%
			} else {
	...
	<form name="comment_frm" action="qna_comment_ok.jsp?qna_no=<%=qna_no%>&question_type=<%= q_type %>" method="post">
		<div>
			<div class="comment-title">전체 댓글 <%=qna_comment_cnt %>개</div>
			<table class="comment-table">
			<%
				for(int i = 0; i < list.size(); i++) {
					BoardBean board = list.get(i);
					String qna_c_user_nick = board.getQna_c_user_nick();
					String qna_c_content = board.getQna_c_content();
					Timestamp qna_c_time = board.getQna_c_time();
					int qna_c_no = board.getQna_c_no();
					if(qna_c_user_nick.equals(logon_nick)) {
			%>
						<tr>
							<td class="comment-table-writer_x"><%=qna_c_user_nick %></td>
							<td class="comment-table-content_x" cols="50" rows="8"><%= qna_c_content %></td>
							<td class="comment-table-time_x"><%= sdf.format(qna_c_time) %>
								<a href="qna_delete_comment_ok.jsp?qna_no=<%= qna_no%>&question_type=<%= q_type %>&qna_c_no=<%= qna_c_no %>">x</a>
							</td>
						</tr>
			<% 	
					} else {
			%>
						<tr>
							<td class="comment-table-writer"><%=qna_c_user_nick %></td>
							<td class="comment-table-content"><%= qna_c_content %></td>
							<td class="comment-table-time"><%= sdf.format(qna_c_time) %></td>
						</tr>
			<%			
			...
			<div class="comment_div">
				<input type="text" name="qna_c_content" class="comment_content"><br>
				<input type="submit" value="댓글 달기" onclick="comment_check()" class="comment_btn">
			</div>
			<%
				//관리자 이외에는 답글 못달도록하니까 너무 밑에 붙어서 관리자 아닌경우에는 답글 밑에 여유공간 줌
				}else {
			%>
			<br><br><br><br><br> 
			<%
