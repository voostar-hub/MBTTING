...
%>
	...
	<header class="board-header">
		<h1>
			<a href="chatting_qna.jsp">Q & A - 채팅 관련</a>
		</h1>
	</header>
	<hr />
	<%
	...
	<div class="if_search_form">
		<p>
			<span>제목</span>에 <span>"<%=content%>"
			</span>(이)가 포함된 게시글 리스트입니다.
		</p>
	</div>
	<%
	...
	<div class="qna_board1">
		<!-- qna side 내용(공지선택) -->
		<div class="qna_side_div">
			<table class="qna-side">
				<tr>
					<td class="side-board-link"><a href="board_qna.jsp">- 게시판 관련 </a></td>
				</tr>
				<tr>
					<td class="side-board-link"><a href="chatting_qna.jsp">- 채팅 관련 </a></td>
				</tr>
				<tr>
					<td class="side-board-link"><a href="etc_qna.jsp">- 기타 </a></td>
				</tr>
			</table>
		</div>
		<div class="qna_main_div">
			<form>
				<table class="t">
					<tr>
					...
					ArrayList<BoardBean> list = db.chattingQnaList(pageNum, col, content);
				
					for (int i = 0; i < list.size(); i++) {
						BoardBean board = list.get(i);
	
						int qna_no = board.getQna_no();
						String qna_title = board.getQna_title();
						String writer = board.getUser_nick();
						Timestamp write_date = board.getDate();
						int qna_count = board.getQna_count();
						String qna_type = board.getQuestion_type();
						int qna_comment_cnt = db.qnacountcomment(qna_no); //총 댓글 갯수
					%>
					<tr>
						<td align="center"><%=qna_no%></td>
						<td>
					<%
						if(qna_comment_cnt == 0) {
					%>
							<a href="qna_show.jsp?qna_no=<%=qna_no%>&question_type=<%=qna_type%>"><%=qna_title%></a>
					<% 
						}else {
					%>		
							<span class="answer1">[답변완료]</span> <a href="qna_show.jsp?qna_no=<%=qna_no%>&question_type=<%=qna_type%>"><%=qna_title%></a>
					<%
						}
					 %> 	
						</td>
				...
				<div class="paging"><%=BoardBean.chatting_qna_pageNumber(5, col, content)%></div>
			</form>
		</div>
	</div>
	<%
		ArrayList<BoardBean> list2 = db.chattingQnaList(pageNum, col, content);
	int qna_no = 0;
	for (int i = 0; i < list2.size(); i++) {
		BoardBean board = list2.get(i);
		qna_no = board.getQna_no();
	}
	if (qna_no != 0) {
	%>
	<div class="board2">
		<form name="qna_search_frm" action="board_qna.jsp" method="post"
			...
			<div class="search_board">
				...
				<input type="button" value="검색" onclick="qna_search_check()" class="search_board_btn">
			</div>
		</form>
	</div>
	<%
			...
			%>
			<input type="button" value="새글 쓰기"
				onclick="location.href='qna_write.jsp'" class="write_btn">
			<%
	%>
	<hr />
	<footer>
