...
%>
    	...
    	<form>
	    	<table class="t">
	    		...
	    		<%
	    			...
	    			ArrayList<BoardBean> list = db.freelistBoard(pageNum, col, content);
				...
				%>
					...
					<% 
								if(comment_cnt == 0) {
					%>
									<a href="../mbtting_login/login.jsp"><%= title %></a>
					<%
								} else {
					%>								
									<a href="../mbtting_login/login.jsp"><%= title %>(<%=comment_cnt %>)</a>
					<% 
								}
					%>
							</td>
		    ...
		    <div class="paging"><%= BoardBean.free_pageNumber(5, col, content) %></div>
    	</form>
    </div>
    <%
   		ArrayList<BoardBean> list2 = db.freelistBoard(pageNum, col, content);
    	int b_no = 0;
	...
	%>
			<div class="board2">
    			<form name="search_frm" action="free_board.jsp" method="post" onsubmit="return false">
    ...
    <%
    	if(nick == null) {
    %>
		    <input type="button" value="새글 쓰기" onclick="location.href='../mbtting_login/login.jsp'" class="write_btn" >
    <%
    	} else {
    %>
