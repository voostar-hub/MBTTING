...
<body>
	...
	<div class="write_form">
		<form name="edit_frm" action="edit_ok.jsp?b_no=<%= b_no %>&board_type=<%=type %>" method="post">
			<div class="writeForm_div">
				...
				<div class="write_selectbox">
					<select class="selectbox" name="board_type">
						...
						<option value="anal" 
								<c:if test="${type == anal}">selected</c:if>>분석형 게시판</option>
						<option value="dipl" 
								<c:if test="${type == dipl}">selected</c:if>>외교형 게시판</option>
						<option value="admin" 
								<c:if test="${type == admin}">selected</c:if>>관리자형 게시판</option>
						<option value="exp" 
								<c:if test="${type == exp}">selected</c:if>>탐험가형 게시판</option>
						<option value="free" 
								<c:if test="${type == free}">selected</c:if>>자유 게시판</option>
					</select>
				</div>
				<div class="title_div">
					<input type="text" name="title" placeholder="내용을 입력해 주세요." value="<%=bb.getTitle() %>" class="title" maxlength="30">
				</div>
				<div class="content_div">
					<textarea name="content" class="content" placeholder="내용을 입력해 주세요."  maxlength="1000"><%= bb.getContent() %></textarea>
				</div>
				<div class="input_div">
					<div class="pwd_div">
					...
					<div class="input_btn_div">
						...
						<input type="button" value="수정 완료" onclick="edit_check()" class="write_ok_btn">
					</div>
				</div>
			</div>
		</form>
